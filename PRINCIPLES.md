...
head: module-boundaries-ci-gate
artifact: principles
...
> Generated by `/module-boundaries`. IDs MB001–MB015. Last updated: 2026-03-23 (ci-gate added). Re-run `/module-boundaries` to refresh.

10 Python packages scanned. `__all__` declared in all 10 packages. No new violations since 2026-03-22.
MB015 added: CI gate (`scripts/check_module_boundaries.py`) now wired into GitHub Actions
(`.github/workflows/module-boundaries-gate.yml`) and GitLab CI (`module-boundaries` job).
...
### 11.5 CI gate enforces boundary rules on every PR (MB015)

**Rationale:** Static principles in `.claude/principles.yaml` are only useful if they
are enforced automatically. `scripts/check_module_boundaries.py` performs a
grep-based deep-import scan at build time; any new violation introduced in a PR
causes the gate to exit 1 and block merge.

**Gate locations:**
- GitHub Actions: `.github/workflows/module-boundaries-gate.yml`
- GitLab CI: `module-boundaries` job (stage: `boundaries`, runs before all other gates)

**Example:**
- DO: Keep the `module-boundaries` CI job enabled; never add `allow_failure: true`.
- DON'T: Skip or comment out the gate to "unblock" a PR — fix the import instead.

**Re-generating the script:** If a new domain package is added, re-run
`/module-boundaries` to update `scripts/check_module_boundaries.py` with the new
domain entry so it is enforced in CI automatically.
