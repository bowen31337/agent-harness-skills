...
|---|---|---|---|---|
| P033 | error-handling | 🔴 blocking | review-pr, check-code | All raised exceptions must be structured domain exceptions subclassing `AppError`; bare `raise Exception(…)` / `raise ValueError(…)` are forbidden in domain code |
| P034 | error-handling | 🔴 blocking | review-pr, check-code | Every domain exception class must declare a unique `error_code` from the central registry (`harness_skills/error_codes.py`) using format `<DOMAIN>_<CATEGORY>_<NNN>` |
| P035 | error-handling | 🔴 blocking | review-pr, check-code | All ERROR-level log records must include `error_code`, `exc_type`, `trace_id`, and `context` fields; free-form `logger.error(str(exc))` without structured fields is forbidden |

*3 error-handling principles active.*
