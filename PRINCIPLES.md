...
## 12. Boundary Validation Rules
> Generated by `/boundary-validation`. IDs BV001–BV005 (structural), BV010–BV011 (per-file violations). Last updated: 2026-03-22. Re-run `/boundary-validation` to refresh.

Data validation must happen exclusively at system boundaries — API route handlers,
CLI argument parsers, and external service adapters. Domain services, use-case classes,
and repositories receive already-validated, fully typed objects and must not re-validate them.

### 12.1 Validate only at system boundaries, never in business logic (BV001)

**Rationale:** Validation code scattered across service and repository layers creates
defensive clutter, slows down tests (which must now provide raw dicts instead of typed
objects), and makes it impossible to identify the single authoritative source of validation
for any given field. Centralising validation at the boundary means invalid input is
rejected as early as possible with a clear, user-facing error message.
...
- `Pydantic model_validate()` / `parse_obj()` calls on untrusted data

**Approved at the boundary layer:**
- Pydantic `BaseModel` parsed at the route or adapter entry point
- FastAPI `Depends()` used to validate before the service is called
- Adapter methods (e.g. `ExternalServiceClient.parse_response()`) that return typed objects

**Example:**
- DO: Parse the request body into a `CreatePlanRequest` Pydantic model in the route function, then pass the typed model to `PlanService.create()`.
- DON'T: Call `plan_service.create(data: dict)` and perform `if "name" not in data: raise ValueError(...)` inside the service method.

---

### 12.2 Validate external service responses inside adapters (BV002)

**Rationale:** External API responses are an untrusted boundary just like user input.
Validating them inside adapters/clients creates a single quarantine zone; all code
beyond the adapter can be written without defensive null-checks.

**Example:**
- DO: `ExternalServiceClient.fetch_user()` validates the raw JSON payload against a schema and returns a typed `User` domain object.
- DON'T: Pass a `requests.Response` or `httpx.Response` object into a service method and call `.json()` inside that method.

---

### 12.3 Never duplicate validation across layers (BV003)

**Rationale:** Duplicated validation rules diverge silently over time. One copy gets
updated and the other does not, creating inconsistent enforcement that is difficult to audit.

**Example:**
- DO: Declare `email: EmailStr` in the Pydantic request schema at the boundary. Trust that field everywhere downstream.
- DON'T: Re-validate `email` format inside `UserService.create_user()` after it was already validated at the route layer.

---

### 12.4 Typed parameters required in service and repository layers (BV004)

**Rationale:** Typed signatures make it structurally impossible to pass unvalidated data
to business logic. Static type checkers (mypy/pyright) catch violations at check time
rather than at runtime.

**Accepted parameter types:** domain-specific dataclasses, Pydantic models, primitive
scalars (`str`, `int`, `float`, `bool`, `Enum` members), and typed collections of the above.

**Forbidden in service/repo signatures:** `dict`, `Any`, `object`, `Union[X, None]`
where the `None` branch implies "not yet parsed".

**Example:**
- DO: `def create_plan(self, request: CreatePlanRequest) -> Plan:`
- DON'T: `def create_plan(self, data: dict) -> dict:`

---

### 12.5 HTTP-framework types must not appear outside the boundary layer (BV005)

**Rationale:** HTTP-agnostic services can be exercised by CLI commands, background
workers, and unit tests without spinning up an HTTP server. Coupling services to
`HTTPException` or `Response` objects defeats this.

**Prohibited in service/repo layers:**
- Importing or raising `HTTPException`, `flask.abort()`, or equivalent
- Returning `Response` / `JSONResponse` / `StreamingResponse` objects
- Referencing `status.HTTP_4xx` / `status.HTTP_5xx` constants

**Required pattern:** Raise a typed domain exception (e.g. `PlanNotFoundError`);
add an exception handler in the route module that maps domain exceptions to HTTP status codes.

**Example:**
- DO: Service raises `PlanNotFoundError("PLAN-42 does not exist")`; route handler catches it and returns `HTTPException(status_code=404, detail=str(e))`.
- DON'T: `raise HTTPException(status_code=404, detail="plan not found")` inside `PlanService.get_by_id()`.

### 12.6 Open violations — fix on next touch

#### BV010 — Duplicate trace_id / domain validation (`harness_skills/logging_config.py` + `harness_skills/models/observe.py`)

**Rule (BV003):** Each field invariant must be validated in exactly one place.

The `_TRACE_ID_RE` regex and the `domain` dot-validation logic are duplicated between
`harness_skills/logging_config.py` (used by `set_trace_id()` and `get_logger()`) and
`harness_skills/models/observe.py` (used by `LogEntry` field validators). Both copies
must stay in sync manually — a classic BV003 violation.

**Resolution:** Extract the regex and domain-validation logic into a shared module
(e.g. `harness_skills/validators.py`) and import from there in both files, or declare
the canonical validators only in `harness_skills/models/observe.py` and import them
into `logging_config.py`.

**Files:**
- `harness_skills/logging_config.py:116-119` — `set_trace_id()` re-validates trace_id format
- `harness_skills/logging_config.py:365-367` — `get_logger()` re-validates domain format
- `harness_skills/models/observe.py:107-123` — `LogEntry` field validators (canonical location)

---

#### BV011 — Service-layer ValueError in config generator (`harness_skills/generators/config_generator.py:266-269`)

**Rule (BV001):** Service-layer functions must not re-validate input already constrained by the CLI boundary.

`generate_gate_config()` raises `ValueError` for an unrecognised profile. The CLI boundary
(`harness_skills/cli/create.py`) already constrains the profile via `click.Choice`, which
rejects invalid values before this function is ever called. The duplicate guard inside the
generator is therefore dead code for the normal code path, and an indication of misplaced
validation for the programmatic API.

**Resolution:** Remove the `ValueError` guard from `generate_gate_config()` for the CLI
path, or move it to an explicit typed input class validated before the generator is invoked.
If the generator is called programmatically without a CLI boundary, add a thin adapter/wrapper
that validates the `profile` argument and delegates to the generator.

**File:** `harness_skills/generators/config_generator.py:266-269`
...
## Concurrency Patterns (CP001–CP005)
> Auto-generated by `/concurrency-patterns` on 2026-03-22.
> Detected profiles: **sdk-async-hooks**, **anyio-first**, **asyncio-native (mixed)**, **posix-file-lock**.
> Re-running `/concurrency-patterns` updates CP principles in `.claude/principles.yaml` and this section.

---

### CP001 — Never mix asyncio and threading locks on shared state  🔴 blocking

**Rationale:** asyncio.Lock and threading.Lock live in different synchronisation
domains. Using threading.Lock inside a coroutine does not yield to the event loop,
causing deadlocks or incorrect mutual-exclusion semantics.

**Example:**
```python
# VIOLATION — threading.Lock inside a coroutine
import threading
_lock = threading.Lock()
async def update(d, k, v):
    with _lock:          # blocks the event loop; wrong primitive
        d[k] = v

# COMPLIANT — asyncio.Lock (or anyio.Lock — see CP004)
import asyncio
class Registry:
    def __init__(self):
        self._lock = asyncio.Lock()   # instance-level (see P025)
    async def update(self, d, k, v):
...
### CP002 — Never call blocking I/O inside an async function  🔴 blocking

**Rationale:** Blocking calls (open, requests, time.sleep, subprocess.run) freeze the
entire event loop for the duration of the call, starving every other coroutine.

**Example:**
```python
# VIOLATION
async def read_config(path: str) -> str:
    return open(path).read()          # blocks the loop

# COMPLIANT — offload to thread via anyio (preferred in this codebase)
import anyio
async def read_config(path: str) -> str:
    return await anyio.to_thread.run_sync(lambda: open(path).read())

# COMPLIANT — asyncio alternative
import asyncio
async def read_config_asyncio(path: str) -> str:
    return await asyncio.to_thread(lambda: open(path).read())
```

---

### CP003 — SDK hook callbacks must be `async def`  🔴 blocking

**Rationale:** The Claude Agent SDK awaits each hook. A `def` (sync) function
returns a coroutine object instead of executing — the hook silently does nothing.

**Example (HookMatcher style — preferred for new hooks):**
```python
from claude_agent_sdk import HookMatcher

class MyFeatureHooks:
    def hooks(self) -> dict:
        return {
            "PostToolUse": [
    ...
    # COMPLIANT — async def, returns dict
...
# VIOLATION — sync hook; the SDK awaits it and gets a coroutine object back
    def _on_post_tool_use(self, input_data, tool_use_id, context):
        ...
```

**Plain-dict style** (as used in `performance_hooks.py`):
```python
def hooks(self) -> dict:
    return {
        "SessionStart":        self._on_session_start,   # async def required
        "PreToolUse":          self._on_pre_tool_use,    # async def required
        "PostToolUse":         self._on_post_tool_use,   # async def required
...
### CP004 — Prefer anyio primitives over bare asyncio  🟡 suggestion

**Rationale:** anyio keeps the codebase backend-agnostic (asyncio + trio). The entry
point already uses `anyio.run(main)` (P029); using bare asyncio inside risks
compatibility issues if the backend ever changes.

**Example:**
```python
import anyio

# Fan-out multiple coroutines — anyio task group (preferred)
async def run_checks() -> None:
    async with anyio.create_task_group() as tg:
        tg.start_soon(check_coverage)
        tg.start_soon(check_docs_freshness)
        tg.start_soon(check_stale_plans)
    # All three finished (or the first exception cancelled the group)

# Offload blocking work — anyio thread bridge (preferred)
async def load_yaml(path: str) -> dict:
    import yaml
    return await anyio.to_thread.run_sync(
        lambda: yaml.safe_load(open(path)), cancellable=True
    )

# Entry point — always anyio.run, never asyncio.run
...
### CP005 — File locks must use `O_CREAT | O_EXCL` (POSIX atomicity)  🔴 blocking

**Rationale:** Check-then-create in two steps is a TOCTOU race: two processes can
both see the file absent and both create it, breaking mutual exclusion.

**Do NOT roll a new implementation.** Use the canonical `TaskLockProtocol` in
`harness_skills/task_lock.py` (see P032 INTER-AGENT COORDINATION section).

```python
# VIOLATION — non-atomic two-step
if not lock_path.exists():
    lock_path.write_text(payload)   # race window here!

# COMPLIANT — atomic single syscall (already in TaskLockProtocol._create_lock_atomic)
import os, errno
try:
    ...
    if exc.errno == errno.EEXIST:
        acquired = False   # another process beat us — back off
    else:
        raise

# Extension / overwrite — atomic rename (already in TaskLockProtocol._overwrite_lock)
tmp = lock_path.with_suffix(".tmp")
tmp.write_text(payload)
tmp.replace(lock_path)              # atomic on POSIX
```

**Lock payload must always include an expiry timestamp:**
```python
{
    "task_id": "feat/my-task",
    "agent_id": "agent-42",
    "acquired_at": "2026-03-22T10:00:00+00:00",
    "expires_at":  "2026-03-22T10:05:00+00:00",   # required — enables TTL sweep
