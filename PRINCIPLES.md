...
head: module-boundaries-rescan-v2
artifact: principles
...
> Generated by `/module-boundaries`. IDs MB001–MB014. Last updated: 2026-03-23 (re-run). Re-run `/module-boundaries` to refresh.

10 Python packages scanned. `__all__` declared in all 10 packages. No new production
violations found since the 2026-03-22 scan. Additional test-layer deep imports catalogued
for MB002 (cli, 13 test violations), MB003 (gates, 5 additional deep imports), and
MB004 (generators, 3 additional test violations). Line shift correction applied to MB005
(`harness_skills/cli/observe.py` moved from :62 → :64).
Re-scan 2026-03-23 (v2): MB003 updated with 3 new inline `check_principles` violations;
MB005 corrected (gate_configs are NOT yet in models `__init__.py`; ~15 new violations added
from test_error_aggregation, test_performance, test_completion_report, test_integration,
gates/__init__.py and gates/performance.py); MB006 updated with test_integration violations.
Open violations remain and are listed per-domain in the principles below — migrate them on next touch.
...
**Status per domain (2026-03-23 scan):**

| Domain | Surface | Open violations |
|--------|---------|-----------------|
| `harness_skills` | ✅ EXPLICIT (`__all__ = []`) | 0 |
| `harness_skills/cli` | ✅ EXPLICIT (5 symbols) | 18 (4 prod: telemetry_reporter/resume bypass cli.fmt/verbosity; 1 test: `_emit` private; 13 test_cli/: command modules not in `__all__`) |
| `harness_skills/gates` | ✅ EXPLICIT (9 public symbols) | 14 (6 deep-import tests: runner/principles/coverage/docs_freshness/security/performance; **+3 new**: inline `check_principles` in test_principles_gate.py:704,719,735; 5 private-symbol test violations) |
| `harness_skills/generators` | ✅ EXPLICIT | 11 (4 cli bypasses + 1 cross-domain; 6 test bypasses; manifest/config symbols absent from `__all__`) |
| `harness_skills/models` | ✅ EXPLICIT (gate_configs **NOT YET** in `__init__`) | ~32 (production + tests; gate_configs still absent from models `__init__`; **+15 new**: gates/__init__.py:33, gates/performance.py:65, test_error_aggregation:92, test_performance:36, test_completion_report:27, test_integration:11-12, cli/completion_report.py; see MB005) |
| `harness_skills/plugins` | ✅ EXPLICIT | 11 (tests bypass + import `_record_telemetry`; **+4 new**: test_integration.py:13-14, gates/runner.py:83-84 bypass) |
| `harness_skills/utils` | ✅ EXPLICIT (`__all__ = []`) | 0 |
| `harness_dashboard` | ✅ EXPLICIT (6 public symbols) | 5 (tests bypass) |
| `dom_snapshot_utility` | ✅ EXPLICIT | 1 (test bypasses `dom_snapshot_utility.snapshot`) |
| `log_format_linter` | ✅ EXPLICIT | 1 (test bypasses `log_format_linter.cli`) |
...
- `tests/gates/test_principles_gate.py:704,719,735` — inline imports of `check_principles` from `harness_skills.gates.runner` (private, not in `__all__`); refactor to test the public `GateEvaluator`/`run_gates` interface instead
...
### 11.3 gate_configs symbols are NOT YET in `harness_skills.models` — but all other deep imports are forbidden (MB005)

**Rationale:** `harness_skills/models/__init__.py` has an explicit `__all__` but it does
**not yet** re-export `gate_configs` types (`CoverageGateConfig`, `PerformanceGateConfig`,
`SecurityGateConfig`, etc.) nor models from `completion`, `status`, `lint`, `evaluate`,
`telemetry`, or `performance`. These must be added to `__init__.py` as a prerequisite
step before callers can be migrated. Until that migration is complete, existing callers
of `harness_skills.models.gate_configs` are grandfathered — but **no new sub-module
imports** of any kind may be introduced.

**Action required:** Add the following submodules' public symbols to `harness_skills/models/__init__.py`:
`gate_configs`, `completion`, `status`, `lint`, `evaluate`, `telemetry`, `performance`, `update`.

**Open submodule violations (all must be migrated once symbols are added to `__init__`):**
- `harness_skills/gates/__init__.py:33` — `from harness_skills.models.gate_configs import SecurityGateConfig`
- `harness_skills/gates/coverage.py:57` — deep into `.models.gate_configs`
- `harness_skills/gates/security.py:54` — deep into `.models.gate_configs`
- `harness_skills/gates/performance.py:65` — `from harness_skills.models.gate_configs import PerformanceGateConfig`
- `harness_skills/gates/runner.py:68,82` — deep into `.models.gate_configs` / `.models.base`
- `harness_skills/generators/config_generator.py:31` — deep into `.models.gate_configs`
- `harness_skills/cli/observe.py:64` — `from harness_skills.models.observe import LogEntry, ObserveResponse`
- `harness_skills/cli/status.py:58-59` — deep into `.models.base` / `.models.status`
- `harness_skills/cli/completion_report.py:63,72` — deep into `.models.completion` / `.models.status`
- `harness_skills/telemetry_reporter.py:39-40` — deep into `.models.base` / `.models.telemetry`
- `harness_skills/stale_plan_detector.py:54-55` — deep into `.models.base` / `.models.stale`
- `harness_skills/plugins/gate_plugin.py:9` — deep into `.models.base`
- `harness_skills/plugins/runner.py:4` — deep into `.models.base`
- `tests/gates/test_principles_gate.py:41`, `tests/gates/test_coverage.py:40`, `tests/gates/test_security.py:76`, `tests/gates/test_performance.py:45` — deep into `.models.gate_configs`
- `tests/test_generators/test_config_generator.py:39` — deep into `.models.gate_configs`
- `tests/test_error_aggregation.py:92` — deep into `.models.errors`
- `tests/test_models/test_performance.py:36` — deep into `.models.performance`
- `tests/test_cli/test_completion_report.py:27` — deep into `.models.completion`
- `tests/plugins/test_integration.py:11-12` — deep into `.models.base` / `.models.evaluate`
- `tests/test_telemetry_reporter.py:25`, `tests/plugins/test_runner.py:10`, `tests/plugins/test_gate_plugin.py:11`, `tests/test_stale_plan_detector.py:12` — deep into `.models.base`

**Example:**
- DO: `from harness_skills.models import CoverageGateConfig` *(after symbols are added to `__init__`)*
- DON'T: `from harness_skills.models.gate_configs import CoverageGateConfig`
- DON'T: introduce any new `from harness_skills.models.<module>` import not already in this list
