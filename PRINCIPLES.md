...
10 Python packages scanned. `__all__` declared in all 10 packages. One new production
violation discovered in MB003/MB005 (gates/__init__.py introduced a direct deep import
of `harness_skills.models.gate_configs` in the 2026-03-23 commit). MB003 listing updated
to include `PrinciplesGate`, `PrinciplesGateConfig`, `SecurityGate` (added to gates __all__).
...
| `harness_skills/gates` | ✅ EXPLICIT (12 public symbols) | 6 (1 NEW: gates/__init__.py:33 deep-imports models.gate_configs; 1 deep import in gates/runner; 4 private-symbol tests) |
...
| `harness_skills/models` | ✅ EXPLICIT (gate_configs TODO — NOT yet re-exported) | ~18 (production + tests; +1 new gates/__init__.py violation) |
...
### 11.3 gate_configs symbols must be migrated to `harness_skills.models` — package root not yet complete (MB005)

**Rationale:** All `gate_configs` types (`CoverageGateConfig`, `RegressionGateConfig`,
`SecurityGateConfig`, `PerformanceGateConfig`, `PrinciplesGateConfig`, etc.) must
ultimately be re-exported from `harness_skills/models/__init__.py`. **They are NOT yet
there** — this is an open TODO. The fix requires adding them to `__all__` in
`harness_skills/models/__init__.py` first, then migrating all callers away from the
direct `harness_skills.models.gate_configs` import path. No new `from
harness_skills.models.gate_configs import` lines may be added until that migration is
complete.
