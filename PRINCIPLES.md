...
head: module-boundaries-one-way-dependency
artifact: principles
...
> Generated by `/module-boundaries`. IDs MB001–MB015. Last updated: 2026-03-23 (re-run). Re-run `/module-boundaries` to refresh.

10 Python packages scanned. `__all__` declared in all 10 packages. MB015 added: explicit
one-way dependency rule documenting the full layer ordering and three open upward-import
violations in `telemetry_reporter`, `resume`, and `models/__init__`.
...
**Status per domain (2026-03-22 scan):**

| Domain | Surface | Open violations |
|--------|---------|-----------------|
| `harness_skills` | ✅ EXPLICIT (`__all__ = []`) | 0 |
| `harness_skills/cli` | ✅ EXPLICIT (5 symbols) | 5 (telemetry_reporter + resume bypass cli.fmt/cli.verbosity; test imports `_emit`; +2 line # shifts) |
| `harness_skills/gates` | ✅ EXPLICIT (9 public symbols) | 5 (1 deep import in gates/runner + 4 private-symbol tests) |
| `harness_skills/generators` | ✅ EXPLICIT | 8 (cli: lint/evaluate/create/manifest bypasses + 3 test bypasses; manifest/config symbols absent from `__all__`) |
...
### 11.5 One-way dependency rule — each layer may only import from layers below it (MB015)

**Rationale:** Layered architectures enforce a single direction of dependency so that
lower-level modules stay unaware of higher-level concerns. When a module in layer N
imports from layer N+1 (upward) or from a peer at layer N (sideways), it creates
circular-dependency risk, makes isolated testing harder, and blurs the responsibility
boundary. The rule is: *a layer may depend on the layer below it, never on the layer
above or beside it.*

**Permitted dependency graph — `harness_skills` sub-packages (top → bottom):**

```
cli          ──→  models, generators, gates, plugins, utils
gates        ──→  models, plugins
generators   ──→  models
plugins      ──→  models
models       ──→  (nothing else inside harness_skills)
utils        ──→  (nothing else inside harness_skills)
```

**Permitted dependency graph — top-level packages (top → bottom):**

```
harness_skills       ──→  dom_snapshot_utility
harness_dashboard    ──→  (standalone)
log_format_linter    ──→  (standalone)
dom_snapshot_utility ──→  (standalone foundation)
```

**Forbidden directions:**
- `models` importing from `cli`, `gates`, `generators`, or `plugins`
- `plugins` importing from `gates` or `generators`
- `generators` importing from `gates` or `cli`
- `gates` importing from `cli` or `generators` (sideways)
- `dom_snapshot_utility` importing from `harness_skills`
- `harness_dashboard` or `log_format_linter` importing from `harness_skills`

**Open violations (as of 2026-03-23):**

| File | Line | Import | Direction | Fix |
|------|------|--------|-----------|-----|
| `harness_skills/telemetry_reporter.py` | 37–38 | `from harness_skills.cli.fmt / cli.verbosity` | root-level → cli (upward) | Move fmt helpers to `harness_skills/utils/fmt.py` |
| `harness_skills/resume.py` | 483–484 | `from harness_skills.cli.fmt / cli.verbosity` | root-level → cli (upward) | Same fix as above |
| `harness_skills/models/__init__.py` | 19 | `from harness_skills.task_lock` | models → root (sideways) | Move `task_lock` into `models` or `utils` sub-package |

**Example:**
```python
# DO — cli imports from the generators layer below it
from harness_skills.generators import run_all_gates

# DON'T — root-level module importing from cli (upward violation)
from harness_skills.cli.verbosity import VerbosityLevel  # in telemetry_reporter.py

# DON'T — models importing from the harness root (sideways)
from harness_skills.task_lock import TaskLock  # in models/__init__.py
```

**Enforcement:** `/check-code` and `/review-pr` flag any new import whose source layer
is at or above the importing module's layer. When uncertain, use the graph above as
the authoritative reference; if a symbol is needed across layers, move it down to
the lowest layer that logically owns it.
