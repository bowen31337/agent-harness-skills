...
<!-- boundary-validation: last_run: 2026-03-24 ids: BV001-BV012 -->
...
| **BV001** | **architecture** | 🔴 **blocking** | review-pr, check-code | **Boundary-only validation:** isinstance() / ValueError / raw-dict .get() prohibited in gate, generator, and plugin layers |
| **BV002** | **architecture** | 🔴 **blocking** | review-pr, check-code | **External data adapter pattern:** raw tool output (pip-audit, coverage, perf YAML) must be validated in a dedicated `*Parser` adapter before reaching a gate |
| **BV003** | **architecture** | 🔴 **blocking** | review-pr, check-code | **No duplicate validation:** each structural invariant validated in exactly one place (the boundary adapter/schema); downstream layers trust typed objects |
| **BV004** | **architecture** | 🔴 **blocking** | review-pr, check-code | **Typed gate parameters:** `dict`, `Any`, `list` (untyped), or `Optional[dict]` forbidden in gate/generator/plugin method signatures |
| **BV005** | **architecture** | 🔴 **blocking** | review-pr, check-code | **Domain exceptions for state invariants:** bare `ValueError`/`TypeError` forbidden in service layer; use structured domain exception classes (see P021) |
| BV010 | architecture | 🔴 blocking | review-pr, check-code | `harness_skills/gates/security.py` lines 352–377: seven isinstance() checks on raw audit JSON must move to a dedicated `AuditReportParser` adapter |
| BV011 | architecture | 🔴 blocking | review-pr, check-code | `harness_skills/gates/performance.py` lines 350–610: isinstance() checks on raw YAML/JSON must move to a `PerformanceReportParser` adapter |
| BV012 | architecture | 🔴 blocking | review-pr, check-code | `harness_skills/gates/runner.py` lines 283–639: isinstance() on raw YAML config must be eliminated via `HarnessConfigLoader` typed config objects |

*43 principles active (35 P/MB + 8 BV).*
...
> Generated by `/boundary-validation`. IDs BV001–BV012. Last updated: 2026-03-24. Re-run `/boundary-validation` to refresh.

**Layer mapping (detected 2026-03-24):**

| Layer | Directories | Status |
|-------|-------------|--------|
| boundary | `harness_skills/cli/`, `skills/` | ✅ correct — CLI entry points |
| service | `harness_skills/gates/`, `harness_skills/generators/`, `harness_skills/plugins/` | ❌ 14 misplaced validation hits |
| repository | (none) | — |
| model | `harness_skills/models/` | ✅ correct — Pydantic at boundary |

**Scan summary (2026-03-24):** 14 isinstance() checks found in gate service layer (security.py ×7, performance.py ×4, runner.py ×3). 0 HTTP-type boundary-leak violations. Note: gates/principles.py isinstance() calls on ast.* types are AST traversal — not data-validation violations.

---

### 12.1 Validation lives at system boundaries only (BV001)

**Rationale:** Gate implementations (`harness_skills/gates/`) are service-layer components: they execute quality-check logic, not input parsing. When a gate validates the shape of raw data it receives, it conflates two responsibilities — parsing and checking — making it impossible to test the gate's logic in isolation and spreading defensive clutter through the codebase. All structural and type checks belong in the boundary adapter that produces the typed object the gate receives.

**Example:**
- DO: CLI evaluate.py calls `AuditReportParser.parse(raw_json)` which raises `ParseError` on malformed input and returns `list[VulnerabilityRecord]`. `SecurityGate.run(records)` receives typed records and contains no isinstance() checks.
- DON'T: `SecurityGate._run()` contains `if not isinstance(data, list): return` directly on the raw pip-audit output.

---

### 12.2 External tool output goes through a dedicated adapter (BV002)

**Rationale:** pip-audit JSON, npm-audit JSON, coverage XML/LCOV/JSON, and performance YAML are external formats that can change between tool versions. Centralising their parsing into a `*Parser` or `*Adapter` class means the gate logic never needs to change when the format evolves — only the parser does. It also makes the gate unit-testable without spawning the external tool.

**Required adapter structure:**
```python
# Compliant — adapter validates and types external tool output
class AuditReportParser:
    def parse(self, raw: list) -> list[VulnerabilityRecord]:
        """Validate raw pip-audit JSON and return typed domain objects.
        Raises ParseError if the format is unrecognised."""
        if not isinstance(raw, list):
            raise ParseError("pip-audit output must be a JSON array")
        return [self._parse_package(pkg) for pkg in raw]

    def _parse_package(self, pkg: dict) -> VulnerabilityRecord:
        # ... validates individual package dict, returns typed object
```

```python
# Violation — gate validates raw data inline
class SecurityGate:
    def run(self, data: dict) -> GateResult:
        if not isinstance(data, list):   # ❌ should not be here
            return GateResult(status=Status.FAILED, ...)
```

---

### 12.3 No duplicate validation across layers (BV003)

**Rationale:** Validating the same field at the CLI layer *and* again inside the gate means the constraint exists in two places. When the constraint changes, both must be updated — or they silently diverge. The boundary schema is the single source of truth; downstream code inherits the constraint by receiving already-validated typed objects.

**Example:**
- DO: `HarnessConfigLoader` validates that `threshold` is a float between 0 and 100. `CoverageGate.run(config: CoverageGateConfig)` accesses `config.threshold` without any range check because the model guarantees it.
- DON'T: `CoverageGate.run()` additionally contains `if not 0 <= threshold <= 100: raise ValueError(...)` — the `CoverageGateConfig` Pydantic model already enforces this with `Field(ge=0, le=100)`.

---

### 12.4 Gate and generator method signatures use typed parameters (BV004)

**Rationale:** A parameter typed as `dict` or `Any` is an implicit contract that the caller may pass anything. This forces the function body to perform runtime type-checking, replicating the boundary problem inline. Typed Pydantic model or dataclass parameters make the contract explicit and verifiable by mypy/pyright.

**Example:**
- DO: `def run(self, config: CoverageGateConfig, report: CoverageReport) -> GateResult:`
- DON'T: `def run(self, config: dict, report: Any) -> GateResult:`

---

### 12.5 Service-layer state errors use domain exceptions, not bare ValueError (BV005)

**Rationale:** `ValueError` and `TypeError` are Python built-ins with no domain semantics. When a task-state constraint is violated (e.g. task already done, lock held by another agent), the error must carry machine-readable context (`code`, `task_id`, `holder`) so the boundary layer can produce a structured error response and tests can assert on the exact failure condition.
...
# After (compliant)
from harness_skills.errors import TaskStateError, ErrorCode

if task["lock_status"] == "done":
    raise TaskStateError(
        code=ErrorCode.HARNESS_TASK_ALREADY_DONE,
        message=f"Task {task_id} is already done — cannot claim.",
        context={"task_id": task_id, "current_status": "done"},
    )
```

Files to migrate: `skills/exec_plan.py` (lines 247–295), `skills/shared_state.py` (line 152), `skills/debt_tracker.py` (line 68).

---

### 12.6 Open per-file refactoring tasks (BV010–BV012)

| Principle | File | Hits | Remediation |
|-----------|------|------|-------------|
| BV010 | `harness_skills/gates/security.py` | 7 isinstance() on audit JSON (lines 352–377) | Create `AuditReportParser`; gate receives `list[VulnerabilityRecord]` |
| BV011 | `harness_skills/gates/performance.py` | 4 isinstance() on perf YAML/JSON (lines 350, 390, 398, 610) | Create `PerformanceReportParser`; gate receives `PerformanceReport` |
| BV012 | `harness_skills/gates/runner.py` | 3 isinstance() on raw YAML config (lines 283, 303, 639) | Route all config through `HarnessConfigLoader`; remove isinstance() guards |

**Note:** `harness_skills/gates/principles.py` contains many `isinstance(node, ast.*)` calls — these are **not** violations; they are intentional AST traversal operations, not data-validation checks.

Re-run `/boundary-validation` after fixing the above to confirm the violation count drops to zero.
