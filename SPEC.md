...
All services write **NDJSON** (newline-delimited JSON) to **stdout** or to a local
log file.  In local development a lightweight file-based collector (Vector)
tails those files, ships entries to a local Loki instance, and makes them
queryable in a browser-based Grafana dashboard — no cloud account required.
...
| `WARN`  | Recoverable anomaly; service continues but investigation advised |
...
Identifies the originating service scope of the log entry.  Structure from coarse to fine:
...
**W3C interop — extract from an incoming `traceparent` header:**
```
...
with dynamic values moved to `extra` keys to enable reliable log grouping.
