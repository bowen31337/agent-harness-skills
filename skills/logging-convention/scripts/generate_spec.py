...
def generate_spec(version: str = CONVENTION_VERSION_DEFAULT) -> str:
    ...
    w(
        ...
        "Every log entry emitted by any service, library, or script **MUST** conform to",
        "this specification. Non-conforming entries will be rejected by the CI linter.",
        "",
        "---",
        "",
    )
    ...
    w(
        ...
        "",
        "| Validity | Value | Reason |",
        "|----------|-------|--------|",
        '| ✅ Valid   | `"2026-03-22T14:22:05.123Z"` | Correct ISO-8601 UTC |',
        ...
        "",
        "Identifies the originating service scope. Structure from coarse to fine:",
        "",
        "```",
        ...
        "| `http.server` | HTTP layer (middleware-generated entries) |",
        "",
        "| Validity | Value | Reason |",
        "|----------|-------|--------|",
        '| ✅ Valid   | `"payments.stripe.webhook"` | Valid dot-separated domain |',
        ...
        "MUST share the same `trace_id`, enabling log correlation across services.",
        "",
        "**Auto-generation:** When the caller does not supply a `trace_id`, the library",
        "generates a random one using a cryptographically secure source.",
        "",
        "**W3C interop — extract from `traceparent` header:**",
        "```",
        "traceparent: 00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01",
        "                 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^",
        "                 use these 32 chars as trace_id",
        "```",
        "",
        "| Validity | Value | Reason |",
        "|----------|-------|--------|",
        '| ✅ Valid   | `"4bf92f3577b34da6a3ce929d0e0e4736"` | 32 lowercase hex chars |',
        ...
        "",
        "// ❌ Discouraged — dynamic message makes aggregation harder",
        '{ "message": "Charge of $19.99 USD succeeded for customer cus_abc123" }',
        "```",
        "",
        "| Validity | Value | Reason |",
        "|----------|-------|--------|",
        '| ✅ Valid   | `"Charge succeeded"` | Non-empty string |',
        '| ❌ Invalid | `""` | Empty string |',
    ...
    w(
        ...
        "|---|-----------|-----------------|---------------|",
        '| 1 | Missing `trace_id` | *(absent)* | `trace_id is required` |',
        '| 2 | Naive timestamp | `"2026-03-22T14:22:05.123"` | `timestamp must be ISO-8601 UTC (trailing Z required)` |',
        ...
        '| 6 | Whitespace message | `"   "` | `message must be a non-empty string` |',
        "",
        "---",
        "",
    )
    ...
    w(
        "## JSON Schema",
        "",
        "The canonical schema is published at `schemas/log_entry.schema.json` and at",
        "the stable public URL `docs/schema/log_entry.schema.json`.",
        "",
        "```json",
        schema_json,
        "```",
...
def main() -> int:
    ...
    if args.format == "json":
        ...
        print("    Draft   : 2020-12")
        print("    Required: timestamp · level · domain · trace_id · message")
        return 0
    ...
    if args.stdout:
        ...
        print("    Fields   : timestamp · level · domain · trace_id · message")
