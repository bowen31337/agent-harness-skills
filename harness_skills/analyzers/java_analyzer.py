"""Java codebase analyzer using regex."""
...
_METHOD_RE = re.compile(r"public\s+(?:static\s+)?[\w<>\[\]]+\s+(\w+)\s*\(")
_CONST_RE = re.compile(r"public\s+static\s+final\s+\w+\s+(\w+)")
...
class JavaAnalyzer(BaseAnalyzer):
    ...
    def analyze(
        ...
        return AnalysisResult(
            language="java", imports=all_imports, symbols=all_symbols,
        )
